...
import { UserCircle, CheckCircle, XCircle, ShieldX } from 'lucide-react';
...
export function ProfileStatsCards({ filters, onFilterChange }: ProfileStatsCardsProps) {
  const { data } = useProfileStats();
  if (!data) return null;

  const cards = [
    { label: 'Total', value: data.total, icon: UserCircle, filterValue: null },
