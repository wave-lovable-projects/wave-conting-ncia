...
export function RequestDialog({ open, onOpenChange }: Props) {
  ...
  const reset = () => { setTitle(''); setDescription(''); setType('ACCOUNT'); setPriority('MEDIUM'); setDueDate(undefined); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createMutation.mutateAsync({
        title, description, type: type as any, priority: priority as any,
        status: 'PENDING', requesterId: 'u1', requesterName: 'Admin Wave',
      ...
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button onClick={handleSubmit} disabled={createMutation.isPending} className="flex-1">Criar</Button>
          </div>
        </div>
      </SheetContent>
    </Sheet>
